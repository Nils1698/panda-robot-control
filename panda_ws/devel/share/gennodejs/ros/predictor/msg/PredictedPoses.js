...
class PredictedPoses {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.poses[i] = geometry_msgs.msg.PoseWithCovariance.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    length += 344 * object.poses.length;
    return length + 4;
  }

  static datatype() {
    // Returns string type for a message object
    return 'predictor/PredictedPoses';
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.poses !== undefined) {
      ...
      for (let i = 0; i < resolved.poses.length; ++i) {
        resolved.poses[i] = geometry_msgs.msg.PoseWithCovariance.Resolve(msg.poses[i]);
      }
    }
    else {
      resolved.poses = []
