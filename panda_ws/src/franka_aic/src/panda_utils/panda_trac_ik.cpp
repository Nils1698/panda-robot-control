...
// ~/ckk_ws/src/tmp/panda_arm.urdf
// /franka_description/robots/panda_arm.urdf.xacro
// /robot_description
// /opt/ros/noetic/share/franka_description/robots/panda_arm.urdf.xacro
                             ...
                             _error(1e-6), _num_steps(10)
{
    ...
    ROS_INFO("Hejsa %s",_urdf_param_string.c_str()); // for debugging
    _panda_trac_ik_solver = std::make_shared<TRAC_IK::TRAC_IK>(name + "_link0", name + "_link8", _urdf_param_string,_secs_timeout, _error, TRAC_IK::Distance); // Works but doesn't account for gripper location only _link8 position
//      _panda_trac_ik_solver = std::make_shared<TRAC_IK::TRAC_IK>(name + "_link0", name + "_EE", _urdf_param_string,_secs_timeout, _error, TRAC_IK::Distance); // Chain between _link8 and _EE could not be found
//      _panda_trac_ik_solver = std::make_shared<TRAC_IK::TRAC_IK>(name + "_link0", name + "_NE", _urdf_param_string,_secs_timeout, _error, TRAC_IK::Distance); // Chain between _link8 and _EE could not be found
//      _panda_trac_ik_solver = std::make_shared<TRAC_IK::TRAC_IK>(name + "_link0", name + "_K", _urdf_param_string,_secs_timeout, _error, TRAC_IK::Distance);

    // _panda_trac_ik_solver = std::make_shared<TRAC_IK::TRAC_IK>(name + "_link0", name + "_link8", _urdf_param_string,_secs_timeout, _error, TRAC_IK::Distance);
