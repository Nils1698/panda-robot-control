...
class PredictionHandler{
  ...
  void msgCallback(const predictor::PredictedPoses& msg) 
  {
    ...
    if (std::sqrt(std::pow(single_target_pose.pose.pose.position.x,2) + std::pow(single_target_pose.pose.pose.position.y,2)) > 0.8){
      ...
      single_target_pose.pose.pose.position.y     = -0.21;  //msg.poses[target_prediction_step*pred_order + target_prediction_order].pose.position.y;
...
{
    ROS_WARN("IT IS HERE IT IS HERE IT IS HERE");
