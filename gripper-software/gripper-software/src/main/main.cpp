...
PUB::pubsub ps({
    ...
    PUB::optical_publisher("opt1", hw::Fingers::F[0]->opticalSensor),
    PUB::optical_publisher("opt2", hw::Fingers::F[1]->opticalSensor),
    PUB::motor_publisher(hw::motor_ref()),
    PUB::control_publisher(GripperActions::controller),
    PUB::perception_publisher(hw::Fingers::F),
    PUB::system_publisher(),
});
...
std::vector<UserInput::UserAction> user_actions = {
  ...
  {"sub", {
    {"type"},
    {"category"},
    {"specifier",   UserInput::Parser::parse_arg<int>(PUB::subscription::NO_SPEC)}
  }, [](UserInput::arg_list_t args){
